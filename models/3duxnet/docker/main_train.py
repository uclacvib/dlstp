...
parser.add_argument('--dataset_json_basename', type=str, default='dataset.json', required=True)
parser.add_argument('--out_classes', type=int, default=2, required=True)
...
# disable above, condor will set this.
parser.add_argument('--cache_rate', type=float, default=0.1, help='Cache rate to cache your dataset into GPUs')
parser.add_argument('--num_workers', type=int, default=2, help='Number of workers')
parser.add_argument('--sw_batch_size', type=int, default=1, help='sw_batch_size')

args = parser.parse_args()

#os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu
#print('Used GPU: {}'.format(args.gpu))
print('Used GPU: {}'.format(os.environ["CUDA_VISIBLE_DEVICES"]))
...
train_ds = CacheDataset(
    ...
    cache_rate=args.cache_rate, cache_num=24,
...
val_ds = CacheDataset(
    ...
    cache_rate=args.cache_rate, cache_num=4,
    num_workers=args.num_workers)
val_loader = DataLoader(val_ds, batch_size=args.batch_size, num_workers=args.num_workers,pin_memory=False,collate_fn=pad_list_data_collate)
...
def train(global_step, train_loader, dice_val_best, global_step_best):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if (
            ...
            json_file = os.path.join(root_dir,f'history-{tstamp}.json')
            ...
            torch.save(
                model.state_dict(), os.path.join(root_dir, "checkpoint_latest.pth")
            )
